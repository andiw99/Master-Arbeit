...
def main():
    ...
    fig2, ax2 = plt.subplots(1, 1, figsize=(7, 4.8))
    ...
    for gpu_dir in gpu_dirs:
        ...
        if os.path.isdir(gpu_path):
            ...
            ax.plot(system_sizes, runtime_per_site_per_sim_time, linestyle="", marker="x", label=gpu)
            ax2.plot(system_sizes, np.array(runtimes_per_sim_time) / 1000 / 60, linestyle="", marker="x",
                     label=gpu)
    ...
    x_ticks = [32 * 32, 64 * 64, 128 * 128, 256 * 256, 512 * 512, 1024 * 1024, 2048 * 2048]
    x_tick_labels = ["32 x 32", "64 x 64", "128 x 128", "256 x 256", "512 x 512", "1024 x 1024", "2048 x 2048"]
    ax.set_xscale("log")
    #ax.set_yscale("log")
    ax.set_xlabel("System size n")
    ax.set_ylabel(r"$\frac{T}{t}/ms$", rotation=0, ha="right")
    ax.set_title("Computation time $T/t$ per site for a fixed Simulation time $t=5000$")
    ...
    ax.legend()
    save_plot(root, "Time-per-Site", "pdf")
    ...
    ax2.legend()
    save_plot(root, "Total-time.pdf", "pdf")
    plt.show()

...
def main():
    ...
    h_given = 0.1 * 3.11
    J_given = 3.11
    T_given = np.linspace(0.8 * J_given, 0.05 * J_given, 1000)
    ...
    ax.plot(Tc[:-1], h[:-1] , marker='s', **blue_point_kwargs, markeredgewidth=1, markersize=7, label="$T_c(h)~/~J_\parallel$")
    #ax.plot(Tc3, h3, marker='s', **get_point_kwargs_color(colors[0], markeredgewidth=1), markersize=7, label="$j_\parallel =  110 \cdot 10^3$")
    ...
    xlims = ax.get_xlim()
    ylims = ax.get_ylim()
    ax.plot(T_given_J_given, h_T_given_plot, color="C1", alpha=0.5, label=f"$h / J_\parallel = {h_given / J_given:.1f}$")

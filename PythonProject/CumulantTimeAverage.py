...
def main():
    simulation_folder = '../../Generated content/Silicon/Subsystems/Suite/Test4/Tc/'
    ...
    print("intersections: ", T_cs)
    T_c_error = np.ptp(T_cs)
    ...
    for i, size in enumerate(sorted(results.keys())):
        if i in shown_inds:
            ...
            U_L_plot = np.array(results[size]["U_L"])
            print(T)
            print(U_L)
            ax.plot(T, U_L_plot, linestyle="-", marker="x", color=colors[ind], label=rf"L = {size}", linewidth=linewidth)
            # ax.plot(x_range, U_L_interpolated[i], color=colors[ind], , linewidth=linewidth)
            ind += 1
            if max_T:
                y_upper_lim = np.maximum(np.max(U_L_plot[(min_T < T) & (T < max_T)]), y_upper_lim)
                y_lower_lim = np.minimum(np.min(U_L_plot[(min_T < T) & (T < max_T)]), y_lower_lim)
    ...
    if max_T:
        ...
        ax.set_ylim(y_lower_lim - 0.2 * y_span, y_upper_lim + 0.2 * y_span)
    mark_point(ax, T_c, U_L, label = rf"$T_c = {T_c:.4f}$")
    configure_ax(fig, ax)
    ...
    diff_arr, size_arr = calc_diff_at(T_c, list(results.values())[0]["T"], cum_dic)

...
// unser state_type ist einfach 2d Vektor, lass uns ublas matrix nehmen? Oder normalen vector?
    ...
    */
    ...
    double dVidq(double qij, double qijp1) {
        return J * sin(qij - qijp1);
    }

    double dVdq(double qij, double qijp1, double t) {
        return dVsdq(qij, t) + dVidq(qij, qijp1);
    }
    ...
    void brownian_step(const state_type &x, state_type &dxdt, state_type &theta, double t){
        for(int i = 0; i < n; i++) {
            for(int j=0; j < n; j++) {
                ...
                dxdt[i][j][1] = (-eta) * x[i][j][1] - dVdq(x[i][j][0], x[i][(j + 1) % n][0], t);
                theta[i][j][0] = 0;
...
int main() {
    ...
    for(int i = 0; i < runs; i++) {
        string name = "Brownian Motion Simple Interaction/" + to_string(i);

...
struct gpu_brownian_system {
    ...
    void operator()(const State &x, Deriv &dxdt, Stoch &theta, double t) const {
...
int main() {
    ...
    typedef thrust::device_vector<double> gpu_state_type;
    euler_mayurama_stepper<gpu_state_type, thrust_algebra, thrust_operations > gpu_stepper(N);

    gpu_brownian_system gpu_system(eta, T, 50000);

    gpu_state_type x(N, 0.0);
    for( size_t n=0 ; n<steps ; ++n ) {
        gpu_stepper.do_step(gpu_system, x, dt, n*dt);
        // cout << n*dt << " ";
        // cout << x[0] << " " << x[1] << endl;
    }
    // add to msd and mu
